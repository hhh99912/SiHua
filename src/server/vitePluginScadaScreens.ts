...
function registerEndpoints(middlewares: any) {
  middlewares.use(async (req: any, res: any, next: any) => {
    ...
    try {
      ...
      if (isDataApi) {
        // 1. GET /api/data 或 /api/datasets - 获取 data/ 目录下所有数据集 JSON
        if (method === 'GET' && (pathname === '/api/data' || pathname === '/api/data/' || pathname === '/api/datasets' || pathname === '/api/datasets/')) {
          const result = loadAllDatasetsFromDisk();
          return sendJson(res, 200, result);
        }

        // 2. POST /api/data/save 或 /api/data - 保存单个数据集至 data/<name>.json
        if (method === 'POST' && (pathname === '/api/data/save' || pathname === '/api/data' || pathname === '/api/datasets/save' || pathname === '/api/datasets')) {
          const body = await parseJsonBody(req);
          if (!body.name && !body.id) {
            return sendJson(res, 400, { success: false, error: '数据集名称或ID不能为空' });
          }
          const result = saveDatasetToDisk(body);
          return sendJson(res, 200, result);
        }

        // 3. DELETE /api/data/:name - 删除指定数据集文件
        if (method === 'DELETE') {
          let nameToDelete = parsedUrl.searchParams.get('name') || parsedUrl.searchParams.get('id') || '';
          if (!nameToDelete) {
            const parts = pathname.split('/');
            if (parts.length >= 3) {
              nameToDelete = decodeURIComponent(parts[parts.length - 1]);
            }
          }
          if (!nameToDelete) {
            return sendJson(res, 400, { success: false, error: '未指定要删除的数据集标识' });
          }
          const result = deleteDatasetFromDisk(nameToDelete);
          return sendJson(res, 200, result);
        }

        return sendJson(res, 404, { success: false, error: `未找到数据集接口: ${method} ${pathname}` });
      }
