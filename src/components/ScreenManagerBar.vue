...
<template>
  <div class="h-10 bg-[#132745] border-t border-cyan-500/40 flex items-center justify-between px-3 z-30 select-none font-mono shadow-sm relative">
    ...
    <div class="flex items-center gap-2">
      ...
      <!-- Quick Action Buttons (全部简化为单个纯图标，中文作为浮标悬停提示) -->
      <div class="flex items-center gap-1 pl-1.5 border-l border-cyan-500/30">
        <!-- Add Screen (纯图标) -->
        <button
          @click="handleOpenAddModal"
          class="p-1.5 rounded bg-[#173055] hover:bg-[#1e406f] border border-cyan-500/40 hover:border-cyan-300 text-cyan-200 hover:text-white transition-all cursor-pointer shadow-xs flex items-center justify-center relative group"
          title="新建画面"
        >
          <Plus class="w-3.5 h-3.5 text-cyan-300 stroke-[2]" />
        ...
        <!-- Rename Current Screen (纯图标) -->
        <button
          @click="handleOpenRenameModal"
          class="p-1.5 rounded bg-[#173055] hover:bg-[#1e406f] border border-cyan-500/40 hover:border-cyan-300 text-cyan-200 hover:text-white transition-all cursor-pointer shadow-xs flex items-center justify-center relative group"
          title="重命名画面"
        >
          <Edit3 class="w-3.5 h-3.5 text-cyan-300 stroke-[2]" />
          <span class="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block px-2 py-1 bg-slate-950/95 border border-cyan-500/50 rounded text-[11px] text-cyan-300 whitespace-nowrap shadow-xl z-50">
            重命名当前画面
          </span>
        </button>

        <!-- Duplicate Current Screen (纯图标) -->
        <button
          @click="handleDuplicateCurrent"
          class="p-1.5 rounded bg-[#173055] hover:bg-[#1e406f] border border-cyan-500/40 hover:border-emerald-400/60 text-cyan-200 hover:text-emerald-200 transition-all cursor-pointer shadow-xs flex items-center justify-center relative group"
          title="复制画面"
        >
          <Copy class="w-3.5 h-3.5 text-emerald-300 stroke-[2]" />
        ...
        <!-- Delete Current Screen (纯图标) -->
        <button
          v-if="screens.length > 1"
          @click="handleDeleteCurrent"
          class="p-1.5 rounded bg-[#173055] hover:bg-[#1e406f] border border-cyan-500/40 hover:border-rose-400/60 text-cyan-200 hover:text-rose-200 transition-all cursor-pointer shadow-xs flex items-center justify-center relative group"
          title="删除画面"
        >
        ...
        <!-- Save As Template (存为模板到 model/ - 纯图标) -->
        <button
          @click="handleOpenSaveTemplateModal"
          class="p-1.5 rounded bg-[#173055] hover:bg-[#1e406f] border border-cyan-500/40 hover:border-amber-400 text-cyan-200 hover:text-amber-200 transition-all cursor-pointer shadow-xs flex items-center justify-center relative group"
          title="存为模板"
        >
          <FolderDown class="w-3.5 h-3.5 text-amber-300 stroke-[2]" />
          <span class="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block px-2 py-1 bg-slate-950/95 border border-amber-500/50 rounded text-[11px] text-amber-300 whitespace-nowrap shadow-xl z-50">
            将当前画面的完整图元组装保存为模板 (model/ 目录)
          </span>
        </button>

        <!-- Login Index Screen Toggle Button (纯图标) -->
        ...
        <button
          ...
          title="设为登录主索引画面"
        >
          <Star class="w-3.5 h-3.5 text-amber-300 stroke-[2]" />
          <span class="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block px-2 py-1 bg-slate-950/95 border border-amber-500/50 rounded text-[11px] text-amber-300 whitespace-nowrap shadow-xl z-50">
            将当前画面设为用户登录后默认展示的主索引大屏
          </span>
        </button>
        <div
          v-else
          class="p-1.5 rounded bg-amber-950/60 border border-amber-400 text-amber-300 shadow-[0_0_8px_rgba(245,158,11,0.25)] flex items-center justify-center relative group"
          title="当前画面已是登录主索引大屏"
        >
          <Star class="w-3.5 h-3.5 text-amber-300 fill-amber-300 stroke-[2]" />
          <span class="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block px-2 py-1 bg-slate-950/95 border border-amber-500/50 rounded text-[11px] text-amber-300 whitespace-nowrap shadow-xl z-50">
            当前画面已是登录主索引大屏
          </span>
        </div>
        ...
        <!-- Open Disk Storage Library (纯图标) -->
        <button
          @click="emit('open:disk-storage')"
          class="p-1.5 rounded bg-[#173055] hover:bg-[#1e406f] border border-cyan-500/40 hover:border-cyan-300 text-cyan-200 hover:text-cyan-100 transition-all cursor-pointer shadow-xs flex items-center justify-center relative group"
          title="graph大屏库"
        >
          <HardDrive class="w-3.5 h-3.5 text-cyan-300 stroke-[2]" />
          <span class="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block px-2 py-1 bg-slate-950/95 border border-cyan-500/50 rounded text-[11px] text-cyan-300 whitespace-nowrap shadow-xl z-50">
            管理磁盘 graph/ 目录下的所有独立大屏 ({{ diskFileCount !== undefined ? diskFileCount : screens.length }} 个文件)
          </span>
        </button>

        <!-- Save to Disk Action (仅保存当前这一个大屏 - 纯图标) -->
        <button
          @click="emit('save:current-disk')"
          :disabled="isSavingDisk"
          class="p-1.5 rounded bg-[#173055] hover:bg-cyan-600 hover:text-slate-950 border border-cyan-400 hover:border-cyan-300 text-cyan-200 transition-all cursor-pointer shadow-[0_0_10px_rgba(0,242,255,0.25)] disabled:opacity-50 flex items-center justify-center relative group"
          title="保存当前大屏"
        >
          <Save class="w-3.5 h-3.5 text-cyan-300 stroke-[2]" :class="{ 'animate-spin': isSavingDisk }" />
          <span class="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block px-2 py-1 bg-slate-950/95 border border-cyan-500/50 rounded text-[11px] text-cyan-300 whitespace-nowrap shadow-xl z-50">
            {{ isSavingDisk ? '保存中...' : `保存当前画面「${currentScreen?.name}」到 graph/${currentScreen?.name}.json` }}
          </span>
        </button>
      </div>
    </div>
